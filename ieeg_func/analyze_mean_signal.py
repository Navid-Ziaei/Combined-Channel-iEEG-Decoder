...
class analyze_signal_mean_patient():
    ...
    def moving_average_signalmean(self, plot_elec_avg, elec_com_fifteen, final_time_show, electrode, plot_output):
        ...
        for key in plot_elec_avg.keys():
            ...
            if plot_output:
                ...
                ax.vlines(time_dash, ymin=-1 * np.ones(time_dash.shape), ymax=np.ones(time_dash.shape), colors='red',
                          ls='--',
    ...
    def moving_average_signal_each_patient(self, channel_names, band_all_patient, band_all_patient_nohil, electrode,
                                           feature):
        ...
        for patient in range(len(band_all_patient)):
            electrodes = channel_names[patient]
            if electrode in electrodes:
                num_electrode = channel_names[patient].index(electrode)
