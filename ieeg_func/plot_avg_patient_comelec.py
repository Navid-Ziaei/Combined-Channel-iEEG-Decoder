...
def plot_comm_elec(elec_com_fifteen,band_all_patient,raw_car_all,elec,final_time,fs,band):
    plot_elec_avg = {}
    for i in range(len(elec_com_fifteen)):
        plot_elec_avg[elec_com_fifteen[i]] = 0

    time = np.arange(0, band_all_patient[0][band].shape[0]) / (fs)
    ...
    for key in plot_elec_avg.keys():
        for i in elec[key]:
            ...
            plot_elec_avg[key] = plot_elec_avg[key] + band_all_patient[i][band][:, num_electrode]
        j=j+1
