...
def RMS_signal_comm(raw_car_all_nohil,band_all_patient_nohil,elec_com,electrode):
    fs = 25
    rms = {}
    ...
    for patient in elec_com:
        ...
        signal = band_all_patient_nohil[patient][:, num_electrode]
        rms[patient] = cal_rms(signal, step=30 * fs)
        output_classification(rms[patient][:12], ax, step=30*fs, ax_x=patient, AVG=False, RMS=True)
