...
def plot_pca(channel_names_list, path, feature_matrix, settings):
    ...
    if settings['task'] == 'Question_Answer':
        label = np.zeros(67)
        label[0:15] = 1
        label_legend = ['question', 'answer']
    if settings['task'] == 'Speech_Music':
        ...
        label[0:6] = 1
        label_legend = ['music', 'speech']
    if settings['task'] == 'Singing_Music':
        label = np.zeros(89)
        label[0:73] = 1
        label_legend = ['music', 'singing']
    ...
    for patient in range(num_patient):
        ...
        for electrode in range(len(ch_name)):
            ...
            plt.savefig(path.path_results_get_pca[patient] + 'PCA_p_' + str(patient) + '_electrode=' + ch_name[electrode])
            plt.savefig(
                path.path_results_get_pca[patient] + 'PCA_p_' + str(patient) + '_electrode=' + ch_name[electrode] + '.svg')
            plt.close()

            x2 = TSNE(n_components=2).fit_transform(feature)
            plt.figure(dpi=300)
            for label2, _ in counter.items():
                row_ix = np.where(label == label2)[0]
                plt.scatter(x2[row_ix, 0], x2[row_ix, 1], label=str(label2))
                plt.legend(label_legend)
            plt.title('patient=' + str(patient) + '_electrode=' + ch_name[electrode], fontsize=15)
            plt.savefig(path.path_results_get_pca[patient] + 'TSNE_p_' + str(patient) + '_electrode=' + ch_name[electrode])
            plt.savefig(
                path.path_results_get_pca[patient] + 'TSNE_p_' + str(patient) + '_electrode=' + ch_name[electrode] + '.svg')
            plt.close()
